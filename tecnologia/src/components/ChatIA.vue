<template>
  <div class="cia">
    ...
    <div class="cia-menu-container">
      ...
      >
        <!-- AURA -->
        <span class="cia-aura" aria-hidden="true"></span>
      ...
      <transition name="cia-menu-fade">
        <div v-if="menuAberto" class="cia-menu" role="menu" aria-label="Opções de assistência">
          ...
          <button class="cia-menu-item" role="menuitem" @click="irParaSuporte">
            ...
            {{ $t?.('central.de.suporte') ?? 'Central de Suporte' }}
          </button>
    ...
    <transition name="cia-slide">
      <div v-if="chatAberto" class="cia-janela" role="dialog" aria-modal="true" aria-label="Chat com Consultora IA">
        ...
        <header class="cia-header">
          ...
          <div class="cia-header-avatar" aria-hidden="true">
            <svg width="18" height="18" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <g transform="translate(24,24)">
                <ellipse cx="0" cy="-11" rx="4.8" ry="6.5" fill="currentColor"/>
                <path d="M0,-17.5 C-1.4,-16 -1.4,-13.5 0,-12.5 C1.4,-13.5 1.4,-16 0,-17.5Z" fill="#0A0A0A"/>
              </g>
              <g transform="translate(24,24) rotate(72)">
                <ellipse cx="0" cy="-11" rx="4.8" ry="6.5" fill="currentColor"/>
                <path d="M0,-17.5 C-1.4,-16 -1.4,-13.5 0,-12.5 C1.4,-13.5 1.4,-16 0,-17.5Z" fill="#0A0A0A"/>
          ...
          <div class="cia-header-info">
            <p class="cia-header-nome">{{ $t?.('noir.ia') ?? 'Noir IA' }}</p>
            <p class="cia-header-status">
              <span class="cia-status-dot" aria-hidden="true"></span>
              {{ $t?.('online.consultora') ?? 'Online · Consultora Noir & Or' }}
            </p>
          </div>

          <div class="cia-header-acoes">
            <button
              class="cia-header-btn"
              @click="confirmarLimpar"
              :title="$t?.('limpar.conversa') ?? 'Limpar conversa'"
        ...
        <div class="cia-input-wrap">
          ...
          <input
            v-model="inputMsg"
            :placeholder="$t?.('pergunte.colecao') ?? 'Pergunte sobre nossa coleção...'"
            @keyup.enter="enviar()"
            :disabled="carregando"
            class="cia-input"
            autocomplete="off"
...
const enviar = async (texto) => {
  ...
  try {
    ...
    const { data } = await api.post('/chat', { mensagem: msg, historico })

    if (data.ok && data.resposta) {
      mensagens.value.push({ role: 'assistant', content: data.resposta, isError: false })
    } else {
      throw new Error(data.msg || 'Resposta inválida')
    }
  } catch (error) {
    console.error('Erro no chat:', error)
    const status = error?.response?.status
    let msgErro = 'Desculpe, estou com dificuldades técnicas. Entre em contato pelo WhatsApp para atendimento imediato.'
    if (status === 429) msgErro = 'Muitas mensagens em pouco tempo. Aguarde um momento e tente novamente.'
...
@keyframes auraPulse {
  0%,100% {
    opacity: 0.50;
    box-shadow:
      0 0 18px 6px rgba(200,168,75,0.28),
      0 0 44px 14px rgba(200,168,75,0.10);
  }
  50% {
    opacity: 0.80;
    box-shadow:
      0 0 28px 10px rgba(200,168,75,0.44),
      0 0 64px 22px rgba(200,168,75,0.16);
  }
}
...
@keyframes petalDrift {
  ...
  15%  { opacity: 0.7; }
  100% { opacity: 0; transform: translateY(-28px) translateX(6px) rotate(45deg); }
}
...
.cia-trigger:hover .cia-aura {
  animation: none;
  box-shadow:
    0 0 32px 12px rgba(200,168,75,0.52),
    0 0 72px 26px rgba(200,168,75,0.20);
}
...
.cia-badge {
  ...
  border: 0.5px solid rgba(200,168,75,0.52);
  border-radius: 3px;
  padding: 1px 6px;
  font-family: var(--font-sans, 'Syne', sans-serif);
  font-size: 8px;
...
.cia-menu-realm-line {
  ...
  background: linear-gradient(
    90deg,
    transparent 5%,
    var(--cor-secundaria, rgba(212,175,55,0.6)) 20%,
    var(--cor-primaria, #F5A623) 50%,
    var(--cor-secundaria, rgba(212,175,55,0.6)) 80%,
    transparent 95%
  );
  opacity: 0.65;
}
...
.cia-status-dot {
  ...
  border-radius: 50%;
  background: #2ecc71;
  box-shadow: 0 0 6px rgba(46,204,113,0.75);
  flex-shrink: 0;
}
...
.cia-header-btn:hover {
  border-color: var(--or-gold, #F5A623);
  color: var(--or-gold, #F5A623);
...
.cia-confirm-sim {
  ...
  border: 0.5px solid rgba(239,68,68,0.42);
  padding: 4px 12px;
  font-family: var(--font-sans, 'Syne', sans-serif);
  font-size: 9px;
...
.cia-confirm-nao:hover {
  border-color: var(--or-hair, rgba(245,166,35,0.12));
  color: var(--or-silk-2, rgba(237,232,224,0.50));
}
...
.cia-sug span:not(.cia-sug-gem) { position: relative; z-index: 1; }
...
@media (prefers-reduced-motion: reduce) {
  ...
  .cia-badge,
  .cia-dot-online,
  .cia-bubble--loading span { animation: none !important; }
}
</style>