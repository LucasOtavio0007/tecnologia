<template>
  <main class="gm" ref="gmRef">
        ...
        <div class="gm__hero-right" ref="heroRight">
          ...
          <svg class="gm__hanko" viewBox="0 0 90 90" aria-hidden="true">
            <rect x="4" y="4" width="82" height="82" rx="2"
              fill="none" stroke="rgba(155,16,16,0.4)" stroke-width="1.5"/>
            <text x="45" y="62" text-anchor="middle"
              font-family="'Noto Serif JP',serif"
              font-size="52" font-weight="200"
              fill="rgba(155,16,16,0.35)">勝</text>
          </svg>

          <!-- FRAG COUNTER — substituição dos hero stats simples -->
    ...
    <!-- ══ HUD STATUS BAR — exclusivo gamer ══ -->
    ...
    <div class="gm__latency" aria-hidden="true">
      ...
      <div class="gm__lat-seg">
        <div class="gm__lat-icon">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
        </div>
        <div class="gm__lat-body">
          <div class="gm__lat-label">Polling rate</div>
          <div class="gm__lat-val gm__lat-val--green">1000 Hz</div>
        </div>
      </div>
      <div class="gm__lat-seg gm__lat-seg--ticker">
        <span class="gm__lat-ticker-track">
          <span class="gm__lat-ticker-set" v-for="n in 3" :key="n">
            <span class="gm__lt-item">戦場 WARFORGE</span><span class="gm__lt-sep">✦</span>
            ...
            <span class="gm__lt-item">刀 Suporte 24/7</span><span class="gm__lt-sep">✦</span>
    ...
    <div class="gm__layout">
      ...
      <!-- ══ SIDEBAR ══ -->
      <aside class="gm__sidebar" ref="sidebarEl" :class="{ 'is-open': sidebarOpen }" :style="sidebarStyle">
        ...
        <div class="gm__sb-sep" aria-hidden="true">
          <span class="gm__sb-sep-line"></span>
          <span class="gm__sb-sep-kanji">価</span>
        ...
        <div class="gm__sb-sep" aria-hidden="true">
          <span class="gm__sb-sep-line"></span>
          <span class="gm__sb-sep-kanji">在</span>
          <span class="gm__sb-sep-line gm__sb-sep-line--r"></span>
        </div>
        ...
        <div class="gm__sb-sep" aria-hidden="true">
          <span class="gm__sb-sep-line"></span>
          <span class="gm__sb-sep-kanji">位</span>
          <span class="gm__sb-sep-line gm__sb-sep-line--r"></span>
        </div>

        <!-- TOP GUERREIROS — exclusivo gamer (no lugar de wishlist padrão) -->
        <div class="gm__sb-sec">
          <p class="gm__sb-lbl">
            <span class="gm__sb-lbl-kanji">位</span>Top Guerreiros
          </p>
        ...
        <div class="gm__sb-sep" aria-hidden="true">
          <span class="gm__sb-sep-line"></span>
          <span class="gm__sb-sep-kanji">愛</span>
          <span class="gm__sb-sep-line gm__sb-sep-line--r"></span>
        </div>
        ...
        <div class="gm__sb-watermark" aria-hidden="true">The Art of Digital Warfare</div>
      </aside>
      ...
      <div class="gm__catalogo">
        ...
        <!-- Skeleton -->
        <div v-if="loading" :class="['gm__grid', viewMode==='grid' ? 'gm__grid--grid' : 'gm__grid--list']">
        ...
        <transition name="view-sw" mode="out-in">
          <div v-if="!loading && produtosPaginados.length && viewMode==='grid'" key="grid" class="gm__grid gm__grid--grid">
            <article
            ...
            >
              ...
              <!-- TIER BADGE — exclusivo gamer -->
              <div class="gm__tier-badge" :class="`gm__tier-badge--${getTier(p).cls}`" aria-label="Tier do produto">
                {{ getTier(p).label }}
              </div>
              ...
              <div class="gm__card-img-wrap">
                <div class="gm__card-fallback" aria-hidden="true">
                  ...
                  <span class="gm__card-fb-kanji">{{ p.subcategoria?.[0] || '装' }}</span>
                  <span class="gm__card-fb-cat">{{ p.categoria }}</span>
                </div>
                <!-- Scanline animada na imagem -->
                ...
                <div class="gm__card-quick">
                  ...
                  <button
                    ...
                    :aria-label="isSaved(p._id||p.id) ? 'Remover' : 'Salvar'"
                  >
                    <svg width="11" height="11" viewBox="0 0 24 24"
              ...
              <!-- Info -->
              <div class="gm__card-info">
                <span class="gm__card-cat">{{ p.subcategoria || p.categoria }}</span>
                <p class="gm__card-marca">{{ p.marca }}</p>
                <p class="gm__card-nome">{{ p.nome }}</p>

                <!-- XP BAR — exclusivo gamer -->
                <div class="gm__xp-wrap" aria-label="XP Rating do produto">
                  <div class="gm__xp-label">
                    <span>XP RATING</span>
                    <span>{{ getXP(p) }} / 10</span>
                  </div>
          ...
          <!-- ══ KILL FEED MODE (list) ══ -->
          <div v-else-if="!loading && produtosPaginados.length && viewMode==='list'" key="list" class="gm__killfeed">
            ...
            >
              ...
              <div class="gm__kf-body">
                <p class="gm__kf-cat">{{ p.subcategoria || p.categoria }} · {{ p.marca }}</p>
              ...
              <div class="gm__kf-acao" @click.stop>
                ...
                >
                  <svg v-if="!addedIds.includes(p._id||p.id)" width="11" height="11"
                    viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                    <path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/>
                    <line x1="3" y1="6" x2="21" y2="6"/>
                    <path d="M16 10a4 4 0 0 1-8 0"/>
                  </svg>
                  <svg v-else width="11" height="11" viewBox="0 0 24 24"
                    fill="none" stroke="currentColor" stroke-width="2.5">
                    <polyline points="20 6 9 17 4 12"/>
                  </svg>
                </button>
                <button
                  class="gm__kf-wish"
                  :class="{ 'is-active': wishlist.includes(p._id||p.id) }"
                  @click="toggleWishlist(p._id||p.id)"
                  :aria-label="wishlist.includes(p._id||p.id) ? 'Remover' : 'Favoritar'"
                >
                  <svg width="11" height="11" viewBox="0 0 24 24"
                    :fill="wishlist.includes(p._id||p.id) ? 'currentColor' : 'none'"
                    stroke="currentColor" stroke-width="2">
        ...
        <nav v-if="totalPaginas > 1" class="gm__paginacao" aria-label="Paginação">
          ...
          <template v-for="p in paginasVisiveis" :key="p">
            ...
            <button v-else :class="['gm__pag-btn', { 'is-active': pagina===p }]" @click="irPag(p)" :aria-current="pagina===p ? 'page' : undefined">{{ p }}</button>
          </template>
          <button class="gm__pag-btn gm__pag-btn--arrow"
            :disabled="pagina===totalPaginas" @click="irPag(pagina+1)" aria-label="Próxima">›</button>
        </nav>

        <p class="gm__pag-info">
            ...
            <div class="gm__modal-layout">
              ...
              <div class="gm__modal-img-col">
                <div class="gm__modal-viewer">
                  <div class="gm__modal-fallback" aria-hidden="true">
                    ...
                    <span class="gm__mf-kanji">{{ produtoModal.subcategoria?.[0] || '装' }}</span>
                    <span class="gm__mf-cat">{{ produtoModal.categoria }}</span>
                  </div>
              ...
              <div class="gm__modal-info" ref="modalInfoEl">
                ...
                <div class="gm__modal-ornament" aria-hidden="true">
                  ...
                  <span class="gm__mo-kanji">{{ produtoModal.subcategoria?.[0] || '至高' }}</span>
                  <span class="gm__mo-line gm__mo-line--r"></span>
                </div>

                <div class="gm__modal-eyebrow">
                  <span class="gm__mey-dot"></span>
                  <span class="gm__mey-txt">{{ produtoModal.subcategoria || produtoModal.categoria }}</span>
                  <span class="gm__mey-dot"></span>
                </div>
                ...
                <!-- STATUS DE PING no modal — exclusivo gamer -->
                ...
                <div class="gm__modal-qty-wrap">
                  ...
                  <div class="gm__modal-qty">
                    ...
                    <button class="gm__qty-btn" @click="qtd=Math.min(produtoModal.qtdEstoque||99,qtd+1)" :disabled="qtd>=(produtoModal.qtdEstoque||99)" aria-label="Aumentar">+</button>
                  </div>
                </div>
...
const todasSubcategorias = heroCatsList
...
const heroStripData = [
  ...
  { title: 'Suporte Gamer 24/7', sub: 'Concierge exclusivo', kanji: '誠',
  ...
  { title: 'Garantia Elite', sub: 'Certificado incluso', kanji: '証',
...
const precoMax   = computed(() => Math.max(30000, ...todos.value.map(p => p.preco || 0)))
const estoquesCriticos = computed(() => todos.value.filter(p => p.estoque && p.qtdEstoque && p.qtdEstoque <= 5).length)

const heroStats = computed(() => [
  { idx: '001', label: 'Equipamentos Elite', val: totalGeral.value, kanji: '装', pct: '100%', gold: false },
  { idx: '002', label: 'Com RGB Sync',       val: todos.value.filter(p=>p.rgb).length||0, kanji: '光', pct: '76%', gold: false },
  { idx: '003', label: 'Rank do Arsenal',    val: 'S', kanji: '位', pct: '98%', gold: true },
])

/* ── Tier e XP — exclusivos gamer ── */
function getTier(p) {
...
const produtosFiltrados = computed(() => {
  ...
  if (categoriasAtivas.value.length)
    lista = lista.filter(p => categoriasAtivas.value.includes(p.subcategoria))
...
const produtosRelacionados = computed(() => {
  ...
  return todos.value.filter(p =>
    ...
    (p._id||p.id) !== (produtoModal.value._id||produtoModal.value.id)
  ).slice(0,3)
})

/* ── HUD: timer e variações ao vivo ── */
...
/* ── Persistência ── */
function salvarWishlist() { try { localStorage.setItem('gm_wishlist', JSON.stringify(wishlist.value)) } catch {} }
function carregarWishlist() { try { const r=localStorage.getItem('gm_wishlist'); if(r) wishlist.value=JSON.parse(r) } catch {} }
function salvarSaved() { try { localStorage.setItem('gm_saved', JSON.stringify(savedItems.value)) } catch {} }
function carregarSaved() { try { const r=localStorage.getItem('gm_saved'); if(r) savedItems.value=JSON.parse(r) } catch {} }
...
const contarCat = val => todos.value.filter(p=>p.subcategoria===val).length
...
function limparFiltros() {
  busca.value=''; categoriasAtivas.value=[]; filtroPreco.value=[0, precoMax.value]
  apenasEstoque.value=false; ordenacao.value='relevancia'; pagina.value=1
}
function irPag(p) {
  pagina.value=p
  nextTick(()=>{ gmRef.value?.querySelector('.gm__catalogo')?.scrollIntoView({behavior:'smooth',block:'start'}) })
}
...
function toggleSaved(p) {
  ...
  if (isSaved(key)) { savedItems.value=savedItems.value.filter(s=>(s._id||s.id)!==key); mostrarToast('Removido','info') }
  else { savedItems.value=[...savedItems.value,{...p}]; mostrarToast('後で買う · Salvo ◆') }
}
...
function abrirModal(p) {
  if (!p) return
  produtoModal.value=p; anguloAtivo.value=0; zoomAtivo.value=false; qtd.value=1
...
function fecharModal() {
  ...
  gsap.to(modalEl.value, {opacity:0, scale:.96, y:16, duration:.3, ease:'power2.in',
    onComplete:()=>{ modalAberto.value=false; zoomAtivo.value=false; document.body.style.overflow='' }})
}
...
function animarHero() {
  ...
  if (heroTitulo.value) {
    const lines=heroTitulo.value.querySelectorAll('.gm__t-thin,.gm__t-main,.gm__t-kanji')
...
function initParticles() {
  ...
  for (let i = 0; i < 30; i++) particles.push(makeParticle(w, h))
  const draw = () => {
    ...
    particles.forEach((p,i)=>{
      ...
      g.addColorStop(0,`rgba(193,18,31,${p.alpha})`)
      g.addColorStop(1,`rgba(155,16,16,0)`)
      ctx.arc(p.x,p.y,p.r,0,Math.PI*2)
      ctx.fillStyle = g
      ctx.fill()
      ctx.restore()
      p.x+=p.vx; p.y+=p.vy
      p.alpha += (Math.random()-.5)*0.005
      p.alpha = Math.max(0.03, Math.min(0.4, p.alpha))
...
function makeParticle(w,h,fromBottom=false) {
  return { x:Math.random()*w, y:fromBottom?h+10:Math.random()*h, r:Math.random()*4+1.5, vx:(Math.random()-.5)*.3, vy:-(Math.random()*.4+.1), alpha:Math.random()*.2+.04 }
}
...
onMounted(async()=>{
  ...
  await produtosStore.fetchProdutos({ categoria: 'GAMING' })
  loading.value=false
  ...
  await nextTick()
  measureToolbar(); initTilt(); animarHero(); initParticles(); startHUD()
...
<style scoped>
@import url('https://fonts.googleapis.com/css2?family=Cinzel:wght@400;600;700;900&family=Noto+Serif+JP:wght@200;300;400&family=Syne:wght@300;400;600;700&display=swap');
...
.gm {
  ...
  --gold2:    #F5D78E;
  --gold-dim: #7a6528;
  --silk:     rgba(237,232,224,0.92);
  --silk2:    rgba(237,232,224,0.55);
  --silk3:    rgba(237,232,224,0.22);
  --silk4:    rgba(237,232,224,0.08);
  --hair:     rgba(200,168,75,0.12);
  --hair2:    rgba(200,168,75,0.07);
  --gold3:    rgba(200,168,75,0.05);
...
/* ══ FUNDO ══ */
.gm__bg { position:fixed; inset:0; pointer-events:none; z-index:0; overflow:hidden; }

.gm__kamon { position:absolute; font-family:var(--f-jp); font-weight:200; color:var(--crimson); pointer-events:none; user-select:none; line-height:1; }
.gm__kamon--1 { font-size:clamp(200px,28vw,380px); bottom:-80px; right:-60px; opacity:.025; animation:kamonDrift 22s ease-in-out infinite; }
.gm__kamon--2 { font-size:clamp(120px,16vw,220px); top:10%; left:-40px; opacity:.018; animation:kamonDrift 28s ease-in-out infinite reverse; }
.gm__kamon--3 { font-size:clamp(80px,10vw,140px); top:42%; left:42%; opacity:.013; animation:kamonDrift 18s ease-in-out infinite; }
@keyframes kamonDrift { 0%,100%{transform:translateY(0) rotate(0deg)} 50%{transform:translateY(-20px) rotate(1deg)} }

.gm__realm-line { position:absolute; top:0; left:0; right:0; height:.5px; background:linear-gradient(90deg,transparent 0%,transparent 6%,var(--gold2) 20%,var(--gold) 50%,var(--gold2) 80%,transparent 94%,transparent 100%); opacity:.45; }
.gm__washi { position:absolute; inset:0; opacity:.8; background: repeating-linear-gradient(90deg,rgba(255,255,255,.006) 0,rgba(255,255,255,.006) 1px,transparent 1px,transparent 60px), repeating-linear-gradient(0deg,rgba(255,255,255,.005) 0,rgba(255,255,255,.005) 1px,transparent 1px,transparent 60px); }

/* Scanlines — exclusivo gamer */
.gm__scanlines { position:absolute; inset:0; pointer-events:none; background:repeating-linear-gradient(0deg,transparent 0px,transparent 3px,rgba(0,0,0,.04) 3px,rgba(0,0,0,.04) 4px); opacity:.6; animation:scanlinesDrift 8s linear infinite; }
@keyframes scanlinesDrift { 0%{transform:translateY(0)} 100%{transform:translateY(4px)} }

.gm__fog { position:absolute; pointer-events:none; border-radius:50%; filter:blur(90px); }
.gm__fog--1 { width:70vw; height:70vw; bottom:-30vw; left:-15vw; background:radial-gradient(circle,rgba(155,16,16,.06) 0%,transparent 70%); animation:fogDrift1 20s ease-in-out infinite; }
.gm__fog--2 { width:45vw; height:45vw; top:-12vw; right:10vw; background:radial-gradient(circle,rgba(200,168,75,.03) 0%,transparent 70%); animation:fogDrift2 26s ease-in-out infinite; }
...
.gm__particles { position:fixed; inset:0; z-index:0; pointer-events:none; opacity:.7; }

/* ══ HERO ══ */
.gm__hero { position:relative; z-index:2; padding:calc(var(--navbar-h,88px) + 64px) 80px 0; border-bottom:.5px solid var(--hair2); overflow:hidden; }
.gm__edge-line { position:absolute; left:calc(6% - 1px); top:0; bottom:0; width:.5px; background:linear-gradient(to bottom,transparent 0%,var(--hair) 22%,var(--hair) 78%,transparent 100%); z-index:3; pointer-events:none; }
.gm__edge-num { position:absolute; top:calc(var(--navbar-h,88px)+40px); left:50%; transform:translateX(-50%) rotate(-90deg); font-family:var(--f-mono); font-size:7px; letter-spacing:.3em; color:var(--gold); opacity:.32; white-space:nowrap; }
.gm__hud { position:absolute; right:28px; top:50%; display:flex; flex-direction:column; align-items:center; gap:8px; transform:translateY(-50%) rotate(90deg); z-index:3; pointer-events:none; }
.gm__hud-label { font-size:7px; letter-spacing:.52em; text-transform:uppercase; color:var(--silk4); }
.gm__hud-sep   { width:18px; height:.5px; background:linear-gradient(90deg,transparent,var(--gold),transparent); opacity:.32; }
.gm__hud-coord { font-family:var(--f-mono); font-size:7px; letter-spacing:.28em; text-transform:uppercase; color:var(--gold); opacity:.48; }

.gm__hero-inner { display:grid; grid-template-columns:1fr 300px; gap:64px; align-items:center; padding-left:calc(6% + 28px); }

.gm__kicker { display:flex; flex-direction:column; align-items:flex-start; gap:7px; margin-bottom:22px; }
.gm__kicker-num { font-family:var(--f-mono); font-size:9px; letter-spacing:.3em; color:var(--gold); opacity:.4; }
.gm__kicker-bar { width:.5px; height:38px; background:linear-gradient(to bottom,var(--crimson-h),transparent); opacity:.5; }

.gm__eyebrow { display:flex; align-items:center; gap:10px; margin-bottom:22px; }
.gm__ew-dot  { width:5px; height:5px; border-radius:50%; background:var(--crimson-h); opacity:.8; flex-shrink:0; animation:dotPulse 2.5s ease-in-out infinite; }
.gm__ew-text { font-size:8px; font-weight:600; letter-spacing:.52em; text-transform:uppercase; color:var(--gold); opacity:.7; }
...
.gm__t-thin { font-family:var(--f-ui); font-size:clamp(1.4rem,3.5vw,3rem); font-weight:300; color:var(--silk3); letter-spacing:-.01em; text-transform:uppercase; line-height:1; display:block; }
.gm__t-main { font-family:var(--f-title); font-size:clamp(3rem,9vw,7rem); font-weight:900; background:linear-gradient(135deg,var(--silk) 0%,var(--gold2) 35%,var(--crimson-h) 70%,var(--ember) 100%); background-size:200% auto; -webkit-background-clip:text; background-clip:text; -webkit-text-fill-color:transparent; animation:mainShine 6s linear infinite; line-height:.9; display:block; text-transform:uppercase; letter-spacing:.04em; }
@keyframes mainShine { from{background-position:0% center} to{background-position:200% center} }
.gm__t-kanji { font-family:var(--f-jp); font-weight:200; font-size:clamp(.9rem,1.4vw,1.2rem); letter-spacing:10px; color:var(--gold); opacity:.18; margin-top:10px; display:block; }

.gm__sep { display:flex; align-items:center; gap:10px; width:200px; margin:0 0 18px; }
.gm__sep-line { flex:1; height:.5px; background:linear-gradient(to right,var(--crimson-h),transparent); opacity:.4; }
.gm__sep-line--r { background:linear-gradient(to left,var(--crimson-h),transparent); }
.gm__sep-gem { font-family:var(--f-jp); font-size:13px; color:var(--gold); opacity:.4; line-height:1; flex-shrink:0; }

.gm__sub { font-family:var(--f-mono); font-size:9px; letter-spacing:2px; color:var(--silk4); text-transform:uppercase; margin-bottom:24px; }

/* ══ LOADOUT SLOTS — exclusivo gamer ══ */
.gm__loadout { margin-bottom:24px; }
.gm__loadout-label { display:flex; align-items:center; gap:8px; margin-bottom:10px; }
.gm__ll-kanji { font-family:var(--f-jp); font-size:13px; color:var(--crimson-h); opacity:.65; line-height:1; }
.gm__ll-txt { font-family:var(--f-mono); font-size:6px; letter-spacing:5px; text-transform:uppercase; color:var(--silk3); opacity:.6; }
.gm__loadout-slots { display:flex; flex-wrap:wrap; gap:6px; }
.gm__lslot {
  position:relative; overflow:hidden;
  background:var(--deep); border:.5px solid var(--hair2);
  padding:10px 14px; display:flex; flex-direction:column; align-items:center; gap:3px;
  min-width:68px; cursor:pointer; transition:all .35s var(--ease);
}
/* clip angular no canto */
.gm__lslot::before { content:''; position:absolute; top:0; right:0; width:0; height:0; border-style:solid; border-width:0 12px 12px 0; border-color:transparent var(--void) transparent transparent; z-index:3; transition:border-color .3s; }
.gm__lslot::after  { content:''; position:absolute; top:0; right:12px; width:0; height:0; border-style:solid; border-width:0 0 12px 12px; border-color:transparent transparent rgba(155,16,16,0) transparent; z-index:2; transition:border-color .3s; }
.gm__lslot:hover::after, .gm__lslot.is-equipped::after { border-color:transparent transparent rgba(155,16,16,.3) transparent; }
.gm__lslot-corner-tl { position:absolute; top:3px; left:3px; width:7px; height:7px; border-top:.5px solid rgba(200,168,75,0); border-left:.5px solid rgba(200,168,75,0); transition:border-color .3s; }
.gm__lslot-corner-br { position:absolute; bottom:3px; right:3px; width:7px; height:7px; border-bottom:.5px solid rgba(200,168,75,0); border-right:.5px solid rgba(200,168,75,0); transition:border-color .3s; }
.gm__lslot:hover .gm__lslot-corner-tl, .gm__lslot.is-equipped .gm__lslot-corner-tl { border-color:rgba(200,168,75,.4); }
.gm__lslot:hover .gm__lslot-corner-br, .gm__lslot.is-equipped .gm__lslot-corner-br { border-color:rgba(200,168,75,.4); }
.gm__lslot:hover { background:rgba(155,16,16,.04); border-color:rgba(155,16,16,.3); }
.gm__lslot.is-equipped { background:rgba(155,16,16,.08); border-color:rgba(155,16,16,.4); }
.gm__lslot-kanji { font-family:var(--f-jp); font-size:1.4rem; font-weight:200; color:var(--crimson-h); opacity:.35; line-height:1; transition:opacity .3s; }
.gm__lslot:hover .gm__lslot-kanji, .gm__lslot.is-equipped .gm__lslot-kanji { opacity:1; }
.gm__lslot-label { font-family:var(--f-mono); font-size:6px; letter-spacing:2px; text-transform:uppercase; color:var(--silk3); }
.gm__lslot.is-equipped .gm__lslot-label { color:var(--crimson-h); }
.gm__lslot-equipped-bar { position:absolute; bottom:0; left:0; right:0; height:1.5px; background:linear-gradient(90deg,transparent,var(--crimson-h),transparent); }

/* ══ FRAG COUNTER — hero stats exclusivo ══ */
.gm__hero-right { display:flex; flex-direction:column; gap:4px; position:relative; }
.gm__hanko { position:absolute; right:-16px; top:-24px; width:78px; height:78px; opacity:.8; pointer-events:none; }
.gm__frag-counter { display:flex; flex-direction:column; gap:0; margin-top:16px; border:.5px solid var(--hair2); overflow:hidden; }
.gm__frag-cell { background:rgba(155,16,16,.025); border-bottom:.5px solid var(--hair2); padding:12px 14px; position:relative; overflow:hidden; transition:background .25s; }
.gm__frag-cell:last-child { border-bottom:none; }
.gm__frag-cell:hover { background:rgba(155,16,16,.045); }
.gm__frag-cell::before { content:''; position:absolute; left:0; top:0; bottom:0; width:.5px; background:linear-gradient(to bottom,var(--crimson-h),transparent); opacity:0; transition:opacity .25s; }
.gm__frag-cell:hover::before { opacity:1; }
.gm__frag-idx   { font-family:var(--f-mono); font-size:7px; letter-spacing:3px; color:var(--gold); opacity:.3; margin-bottom:2px; }
.gm__frag-val   { font-family:var(--f-title); font-size:1.7rem; font-weight:700; color:var(--crimson-h); line-height:1; letter-spacing:-.02em; }
.gm__frag-val--gold { color:var(--gold); }
...
.gm__frag-kanji { position:absolute; bottom:4px; right:8px; font-family:var(--f-jp); font-size:26px; color:var(--crimson); opacity:.06; line-height:1; pointer-events:none; }
.gm__frag-bar   { position:absolute; bottom:0; left:0; right:0; height:1.5px; background:rgba(155,16,16,.1); }
.gm__frag-bar-fill { height:100%; background:linear-gradient(90deg,var(--crimson),var(--crimson-h),var(--gold)); width:0; animation:fragFill 1.6s ease-out forwards; animation-play-state:paused; }
.gm__frag-cell:hover .gm__frag-bar-fill { animation-play-state:running; }
@keyframes fragFill { to{ width:var(--fw,100%); } }

/* Strip */
.gm__strip { display:grid; grid-template-columns:repeat(4,1fr); border-top:.5px solid var(--hair2); margin:40px -80px 0; padding:0 80px; position:relative; z-index:2; }
.gm__strip::before { content:''; position:absolute; top:0; left:0; right:0; height:.5px; background:linear-gradient(90deg,transparent,var(--gold2) 20%,var(--gold) 50%,var(--gold2) 80%,transparent); opacity:.25; }
.gm__strip-item { display:flex; align-items:center; gap:11px; padding:16px 20px 16px 0; margin-right:20px; border-right:.5px solid var(--hair2); position:relative; }
.gm__strip-item:last-child { border-right:none; padding-right:0; margin-right:0; }
.gm__strip-icon { width:32px; height:32px; border:.5px solid var(--hair2); display:flex; align-items:center; justify-content:center; color:var(--crimson-h); flex-shrink:0; transition:all .25s; }
.gm__strip-item:hover .gm__strip-icon { border-color:var(--crimson); background:rgba(155,16,16,.06); }
.gm__strip-title { font-size:10px; font-weight:600; color:var(--silk2); letter-spacing:.5px; margin-bottom:2px; }
.gm__strip-sub   { font-family:var(--f-mono); font-size:7.5px; color:var(--silk4); letter-spacing:1px; }
.gm__strip-kanji { position:absolute; right:10px; top:50%; transform:translateY(-50%); font-family:var(--f-jp); font-size:24px; font-weight:200; color:var(--crimson); opacity:.06; pointer-events:none; transition:opacity .3s; }
.gm__strip-item:hover .gm__strip-kanji { opacity:.14; }
...
.gm__hud-bar-scan {
  ...
  background:linear-gradient(90deg,transparent,var(--crimson-h),var(--gold),var(--crimson-h),transparent);
  opacity:.5; animation:hudScan 3.5s linear infinite; pointer-events:none;
}
@keyframes hudScan { 0%{transform:translateX(-100%)} 100%{transform:translateX(100%)} }
.gm__hud-cell { padding:10px 16px; border-right:.5px solid var(--hair2); position:relative; overflow:hidden; transition:background .2s; }
.gm__hud-cell:last-child { border-right:none; }
.gm__hud-cell:hover { background:rgba(155,16,16,.03); }
.gm__hud-cell::after { content:''; position:absolute; bottom:0; left:0; right:0; height:1px; background:var(--crimson-h); transform:scaleX(0); transform-origin:left; transition:transform .4s var(--ease); }
...
.gm__hud-cell-val--timer { color:var(--crimson-h); font-size:13px; }
.gm__hud-cell-kanji { position:absolute; bottom:3px; right:8px; font-family:var(--f-jp); font-size:22px; color:var(--crimson); opacity:.055; line-height:1; pointer-events:none; }
.gm__hud-dot { width:5px; height:5px; border-radius:50%; flex-shrink:0; }
.gm__hud-dot--red  { background:var(--crimson-h); animation:dotBlink 1.4s ease-in-out infinite; }
.gm__hud-dot--gold { background:var(--gold); animation:dotBlink 1.4s ease-in-out infinite; animation-delay:.7s; }
@keyframes dotBlink { 0%,100%{opacity:1} 50%{opacity:.15} }

/* ══ LATENCY STRIP — exclusivo gamer ══ */
.gm__latency {
  background:rgba(5,5,12,.6); border-bottom:.5px solid var(--hair2);
  display:flex; align-items:stretch; overflow:hidden; z-index:9; position:relative;
}
.gm__lat-seg { display:flex; align-items:center; gap:8px; padding:8px 16px; border-right:.5px solid var(--hair2); flex-shrink:0; }
.gm__lat-seg:last-child { border-right:none; }
.gm__lat-icon { width:22px; height:22px; border:.5px solid var(--hair2); display:flex; align-items:center; justify-content:center; flex-shrink:0; color:var(--crimson-h); }
...
.gm__lat-val--green { color:#22c55e; }
.gm__lat-val--gold  { color:var(--gold); }
.gm__lat-bars { display:flex; align-items:flex-end; gap:2px; height:14px; flex-shrink:0; }
.gm__lat-b { width:3px; background:rgba(155,16,16,.2); animation:latBar 1.6s ease-in-out infinite; }
...
@keyframes latBar { 0%,100%{height:3px;background:rgba(155,16,16,.2)} 50%{height:14px;background:var(--crimson-h)} }
/* ticker no latency */
.gm__lat-seg--ticker { flex:1; overflow:hidden; padding:0; min-width:0; }
.gm__lat-ticker-track { display:flex; white-space:nowrap; animation:tickerMove 40s linear infinite; }
.gm__lat-ticker-set   { display:inline-flex; align-items:center; }
.gm__lt-item { font-family:var(--f-mono); font-size:7px; letter-spacing:3px; text-transform:uppercase; color:var(--silk4); padding:0 16px; height:38px; display:flex; align-items:center; flex-shrink:0; border-right:.5px solid var(--hair2); }
...
/* ══ TOOLBAR ══ */
.gm__toolbar { position:sticky; top:0; z-index:50; background:rgba(5,5,12,.95); backdrop-filter:blur(20px); border-bottom:.5px solid var(--hair2); transition:box-shadow .3s; overflow:hidden; }
.gm__toolbar-border { position:absolute; top:0; left:0; right:0; height:.5px; background:linear-gradient(90deg,transparent,var(--gold2) 20%,var(--gold) 50%,var(--gold2) 80%,transparent); opacity:.3; pointer-events:none; }
.gm__toolbar.is-scrolled { box-shadow:0 4px 28px rgba(0,0,0,.3); border-bottom-color:var(--hair); }
...
.gm__search-wrap { display:flex; align-items:center; gap:8px; background:rgba(255,255,255,.02); border:.5px solid var(--hair2); padding:8px 12px; min-width:190px; flex:1; max-width:300px; transition:border-color .25s,box-shadow .25s; }
.gm__search-wrap.is-focus { border-color:var(--hair); box-shadow:0 0 0 3px rgba(155,16,16,.04); }
.gm__search-wrap svg { color:var(--crimson-h); flex-shrink:0; opacity:.6; }
.gm__search-wrap input { background:none; border:none; outline:none; color:var(--silk); font-family:var(--f-mono); font-size:10px; width:100%; letter-spacing:.05em; }
...
.gm__chip { display:flex; align-items:center; gap:6px; background:rgba(155,16,16,.08); border:.5px solid var(--crimson); padding:4px 10px; font-family:var(--f-mono); font-size:7px; letter-spacing:2px; color:var(--crimson-h); cursor:pointer; transition:all .2s; }
.gm__chip:hover { background:rgba(155,16,16,.15); }
...
.gm__toolbar-select { background:rgba(255,255,255,.02); border:.5px solid var(--hair2); padding:7px 10px; color:var(--silk3); font-family:var(--f-mono); font-size:9px; cursor:pointer; outline:none; transition:border-color .2s; }
.gm__toolbar-select:focus { border-color:var(--hair); }
.gm__toolbar-limpar { background:none; border:.5px solid var(--hair2); padding:7px 12px; font-family:var(--f-mono); font-size:7px; letter-spacing:2px; color:var(--silk4); cursor:pointer; transition:all .2s; }
.gm__toolbar-limpar:hover { border-color:var(--hair); color:var(--gold); }
.gm__view-toggle { display:flex; gap:2px; }
.gm__view-toggle button { width:30px; height:30px; background:transparent; border:.5px solid var(--hair2); color:var(--silk4); cursor:pointer; display:flex; align-items:center; justify-content:center; transition:all .2s; }
.gm__view-toggle button:hover { border-color:var(--hair); color:var(--silk3); }
.gm__view-toggle button.is-active { border-color:var(--crimson); color:var(--crimson-h); background:rgba(155,16,16,.08); }
...
/* ══ SIDEBAR ══ */
.gm__sidebar { background:var(--deep); border-right:.5px solid var(--hair2); position:sticky; display:flex; flex-direction:column; transition:transform .35s var(--ease); scrollbar-width:thin; scrollbar-color:rgba(155,16,16,.15) transparent; overflow:hidden; }
.gm__sidebar::-webkit-scrollbar { width:3px; }
.gm__sidebar::-webkit-scrollbar-thumb { background:rgba(155,16,16,.2); border-radius:2px; }
.gm__sb-kamon { position:absolute; bottom:80px; right:10px; font-family:var(--f-jp); font-size:44px; font-weight:200; color:var(--crimson); opacity:.04; pointer-events:none; z-index:0; line-height:1; }
.gm__sb-watermark { text-align:center; padding:12px 16px 16px; margin-top:auto; font-family:var(--f-title); font-size:7px; letter-spacing:.5em; text-transform:uppercase; line-height:1.7; color:var(--crimson); opacity:.09; pointer-events:none; user-select:none; position:relative; z-index:1; }
...
.gm__sb-overlay { display:none; position:fixed; inset:0; background:rgba(0,0,0,.78); z-index:98; opacity:0; transition:opacity .3s; backdrop-filter:blur(6px); }
...
.gm__sb-orn-line { flex:1; height:.5px; background:linear-gradient(90deg,var(--crimson-h),transparent); opacity:.2; }
.gm__sb-orn-line--r { background:linear-gradient(270deg,var(--crimson-h),transparent); }
.gm__sb-orn-kanji { font-family:var(--f-jp); font-size:13px; color:var(--gold); opacity:.3; line-height:1; flex-shrink:0; }
...
.gm__sb-lbl-kanji { font-family:var(--f-jp); font-size:13px; font-weight:200; color:var(--crimson-h); opacity:.7; letter-spacing:0; line-height:1; }
.gm__sb-count { font-family:var(--f-mono); font-size:8px; color:var(--crimson-h); margin-left:5px; }
.gm__sb-sep { display:flex; align-items:center; gap:10px; padding:12px 16px; position:relative; z-index:1; }
.gm__sb-sep-line { flex:1; height:.5px; background:linear-gradient(90deg,transparent,rgba(155,16,16,.2)); }
.gm__sb-sep-line--r { background:linear-gradient(to left,transparent,rgba(155,16,16,.2)); }
.gm__sb-sep-kanji { font-family:var(--f-jp); font-size:11px; font-weight:200; color:var(--gold); opacity:.28; flex-shrink:0; line-height:1; }

/* Sidebar cats com kanji */
.gm__sb-cats { display:flex; flex-direction:column; gap:1px; }
.gm__sb-cat { display:flex; align-items:center; gap:6px; background:none; border:.5px solid transparent; padding:7px 8px; cursor:pointer; width:100%; font-family:var(--f-ui); font-size:10.5px; color:var(--silk3); transition:all .2s; position:relative; }
.gm__sb-cat::before { content:''; position:absolute; left:0; top:6px; bottom:6px; width:.5px; background:var(--crimson-h); opacity:0; transition:opacity .2s; }
.gm__sb-cat:hover { background:rgba(155,16,16,.05); color:var(--silk2); padding-left:12px; }
.gm__sb-cat:hover::before { opacity:.7; }
.gm__sb-cat.is-active { background:rgba(155,16,16,.08); border-color:rgba(155,16,16,.2); color:var(--crimson-h); }
.gm__sb-cat.is-active::before { opacity:1; }
.gm__sb-cat-kanji { font-family:var(--f-jp); font-size:13px; color:var(--crimson-h); opacity:.5; line-height:1; flex-shrink:0; width:18px; }
...
.gm__sb-range { width:100%; height:2px; -webkit-appearance:none; background:rgba(155,16,16,.18); border-radius:2px; cursor:pointer; accent-color:var(--crimson-h); }
.gm__sb-range::-webkit-slider-thumb { -webkit-appearance:none; width:12px; height:12px; border-radius:50%; background:var(--crimson-h); cursor:pointer; border:2px solid var(--void); box-shadow:0 0 6px rgba(193,18,31,.3); }
.gm__sb-toggle-row { display:flex; align-items:center; gap:9px; cursor:pointer; }
.gm__sb-toggle { width:32px; height:18px; border-radius:18px; background:rgba(155,16,16,.08); border:.5px solid var(--hair2); position:relative; transition:background .2s; flex-shrink:0; }
.gm__sb-toggle.is-on { background:var(--crimson-h); border-color:var(--crimson-h); }
.gm__sb-toggle-thumb { position:absolute; top:2px; left:2px; width:12px; height:12px; border-radius:50%; background:var(--void); transition:left .2s var(--ease); box-shadow:0 1px 4px rgba(0,0,0,.3); }
.gm__sb-toggle.is-on .gm__sb-toggle-thumb { left:16px; }
.gm__sb-toggle-row span { font-size:10px; color:var(--silk3); }
.gm__sb-apply { position:relative; overflow:hidden; background:transparent; border:.5px solid var(--crimson-h); font-family:var(--f-mono); font-size:7px; letter-spacing:3px; text-transform:uppercase; color:var(--crimson-h); padding:9px; cursor:pointer; width:100%; margin-top:10px; transition:color .45s; z-index:0; }
...
.gm__sb-limpar { background:none; border:.5px solid var(--hair2); padding:7px; font-family:var(--f-mono); font-size:6px; letter-spacing:3px; color:var(--silk4); cursor:pointer; width:calc(100% - 32px); margin:10px 16px 16px; transition:all .2s; text-transform:uppercase; display:block; }
...
/* Top Guerreiros na sidebar — exclusivo */
...
.gm__rank-row:hover { background:rgba(155,16,16,.04); border-color:var(--hair2); }
.gm__rank-pos { font-family:var(--f-mono); font-size:8px; color:var(--gold); width:18px; flex-shrink:0; opacity:.5; }
.gm__rank-pos--1 { opacity:1; color:var(--gold2); }
...
.gm__rank-bar-wrap { width:44px; height:1.5px; background:rgba(237,232,224,.05); flex-shrink:0; }
.gm__rank-bar-fill { height:100%; background:linear-gradient(90deg,var(--crimson),var(--crimson-h)); }
...
.gm__sb-wl-item:hover { background:rgba(155,16,16,.05); border-color:var(--hair2); }
.gm__sb-wl-img { width:28px; height:28px; flex-shrink:0; background:var(--void); border:.5px solid var(--hair2); position:relative; overflow:hidden; display:flex; align-items:center; justify-content:center; }
...
.gm__btn-filtros-m { display:none; align-items:center; gap:7px; background:var(--deep); border:.5px solid var(--hair2); padding:9px 14px; font-family:var(--f-mono); font-size:8px; letter-spacing:3px; color:var(--silk3); cursor:pointer; margin-bottom:18px; transition:all .2s; position:relative; }
.gm__btn-filtros-m:hover { border-color:var(--crimson); color:var(--crimson-h); }
.gm__btn-filtros-badge { position:absolute; top:-4px; right:-4px; width:10px; height:10px; background:var(--crimson-h); border-radius:50%; border:1px solid var(--void); }
...
.gm__vazio-ico { width:64px; height:64px; border:.5px solid var(--hair2); display:flex; align-items:center; justify-content:center; position:relative; }
.gm__vazio-ico::before { content:''; position:absolute; top:-1px; left:-1px; width:10px; height:10px; border-top:.5px solid var(--crimson-h); border-left:.5px solid var(--crimson-h); }
.gm__vazio-ico::after  { content:''; position:absolute; bottom:-1px; right:-1px; width:10px; height:10px; border-bottom:.5px solid var(--crimson-h); border-right:.5px solid var(--crimson-h); }
.gm__vazio-kanji { font-family:var(--f-jp); font-size:28px; color:var(--crimson-h); opacity:.35; line-height:1; }
...
/* ══ GRID ══ */
.gm__grid--grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(220px,1fr)); gap:0; border-top:.5px solid var(--hair2); border-left:.5px solid var(--hair2); }
...
.gm__card:hover { background:rgba(155,16,16,.02); box-shadow:0 8px 32px rgba(155,16,16,.07),0 2px 8px rgba(0,0,0,.22); border-color:rgba(155,16,16,.25); }
.gm__card--esgotado { opacity:.55; }

/* Clip angular — exclusivo gamer */
.gm__card-clip { position:absolute; top:0; right:0; width:0; height:0; border-style:solid; border-width:0 20px 20px 0; border-color:transparent var(--void) transparent transparent; z-index:5; pointer-events:none; }
.gm__card-clip-line { position:absolute; top:0; right:20px; width:0; height:0; border-style:solid; border-width:0 0 20px 20px; border-color:transparent transparent rgba(155,16,16,.2) transparent; z-index:4; pointer-events:none; transition:border-bottom-color .3s; }
.gm__card:hover .gm__card-clip-line { border-bottom-color:rgba(155,16,16,.45); }

.gm__card-corner { position:absolute; z-index:5; width:10px; height:10px; border-color:rgba(200,168,75,0); border-style:solid; pointer-events:none; transition:border-color .3s; }
...
.gm__card:hover .gm__card-corner { border-color:rgba(200,168,75,.3); }

.gm__card-num { position:absolute; top:8px; left:10px; z-index:4; font-family:var(--f-jp); font-size:11px; color:var(--gold); opacity:.22; line-height:1; pointer-events:none; transition:opacity .3s; }
.gm__card:hover .gm__card-num { opacity:.45; }

/* TIER BADGE — exclusivo gamer */
.gm__tier-badge { position:absolute; top:8px; left:8px; z-index:6; font-family:var(--f-mono); font-size:6px; letter-spacing:2px; padding:2px 7px; text-transform:uppercase; font-weight:700; }
.gm__tier-badge--s { background:linear-gradient(135deg,#7a1111,var(--crimson-h)); color:rgba(237,232,224,.9); border-left:2px solid var(--gold); }
.gm__tier-badge--a { background:rgba(155,16,16,.15); color:var(--crimson-h); border:.5px solid rgba(155,16,16,.4); }
.gm__tier-badge--b { background:rgba(237,232,224,.03); color:var(--silk3); border:.5px solid var(--hair2); }
...
.gm__card-fallback { position:absolute; inset:0; z-index:0; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:6px; background:linear-gradient(135deg,var(--deep) 0%,rgba(155,16,16,.03) 100%); pointer-events:none; }
.gm__card-fb-letra { font-family:var(--f-title); font-size:2.2rem; font-weight:700; color:var(--crimson); opacity:.18; line-height:1; }
.gm__card-fb-kanji { font-family:var(--f-jp); font-size:.9rem; font-weight:200; color:var(--gold); opacity:.12; letter-spacing:2px; }
.gm__card-fb-cat   { font-family:var(--f-mono); font-size:5px; letter-spacing:4px; text-transform:uppercase; color:var(--silk4); }

/* Scanline na imagem do card — exclusivo */
.gm__card-scan { position:absolute; top:0; left:0; right:0; height:2px; background:linear-gradient(90deg,transparent,var(--crimson-h),transparent); opacity:.35; z-index:2; animation:cardScanLine 2.8s linear infinite; pointer-events:none; }
@keyframes cardScanLine { 0%{transform:translateY(0)} 100%{transform:translateY(calc(75vw * 0.75))} }
...
.gm__card:hover .gm__card-img { transform:scale(1.055); }
.gm__card-shine { position:absolute; inset:0; z-index:4; pointer-events:none; opacity:0; }
.gm__card-overlay { position:absolute; inset:0; background:rgba(0,0,0,.5); z-index:3; display:flex; align-items:center; justify-content:center; opacity:0; transition:opacity .35s; }
.gm__card:hover .gm__card-overlay { opacity:1; }
.gm__card-overlay-txt { font-family:var(--f-title); font-size:7px; letter-spacing:5px; color:var(--gold); text-transform:uppercase; transform:translateY(6px); transition:transform .35s; }
.gm__card:hover .gm__card-overlay-txt { transform:translateY(0); }
.gm__card-urgencia { position:absolute; bottom:8px; left:8px; z-index:4; font-family:var(--f-mono); font-size:6px; letter-spacing:1.5px; text-transform:uppercase; background:rgba(0,0,0,.75); border:.5px solid rgba(239,68,68,.4); color:rgba(239,68,68,.85); padding:3px 8px; }
...
.gm__badge--rgb  { background:rgba(200,168,75,.12); color:var(--gold); border:.5px solid var(--gold-dim); }
.gm__badge--esgo { background:rgba(0,0,0,.7); border:.5px solid var(--hair2); color:var(--silk4); }
...
.gm__quick-btn { width:26px; height:26px; background:rgba(0,0,0,.65); backdrop-filter:blur(5px); border:.5px solid var(--hair2); display:flex; align-items:center; justify-content:center; cursor:pointer; transition:all .25s; color:var(--crimson-h); }
.gm__quick-btn:hover,.gm__quick-btn.is-active { border-color:var(--crimson); background:rgba(155,16,16,.25); transform:scale(1.08); }

/* Card info */
.gm__card-info { padding:14px 16px 16px; display:flex; flex-direction:column; gap:2px; flex:1; }
.gm__card-cat   { font-size:7px; letter-spacing:.5em; text-transform:uppercase; color:var(--crimson-h); opacity:.7; font-family:var(--f-title); }
.gm__card-marca { font-family:var(--f-mono); font-size:6.5px; letter-spacing:3px; color:var(--gold-dim); text-transform:uppercase; }
.gm__card-nome  { font-family:var(--f-title); font-size:12.5px; font-weight:600; color:var(--silk); line-height:1.4; min-height:2.8em; flex:1; letter-spacing:.03em; }
...
.gm__xp-track { height:2px; background:rgba(237,232,224,.06); position:relative; overflow:hidden; }
.gm__xp-fill { height:100%; background:linear-gradient(90deg,var(--crimson),var(--crimson-h)); position:relative; transition:width .6s var(--ease); }
.gm__xp-fill::after { content:''; position:absolute; right:-1px; top:-2px; width:4px; height:6px; background:var(--gold); opacity:.85; }

/* Bottom */
.gm__card-bottom { display:flex; align-items:flex-end; justify-content:space-between; margin-top:auto; padding-top:10px; border-top:.5px solid var(--hair2); }
.gm__card-preco  { font-family:var(--f-title); font-size:1.15rem; color:var(--crimson-h); font-weight:700; }
...
.gm__ping-dot { width:5px; height:5px; border-radius:50%; background:#22c55e; animation:dotBlink 1.3s ease-in-out infinite; }

.gm__card-add { position:relative; overflow:hidden; width:34px; height:34px; flex-shrink:0; background:transparent; border:.5px solid var(--crimson); color:var(--crimson-h); cursor:pointer; display:flex; align-items:center; justify-content:center; transition:color .45s; z-index:0; }
...
.gm__card-add:hover:not(:disabled) .gm__card-add-fill,.gm__card-add.is-added .gm__card-add-fill { transform:scale(1.5); border-radius:0; }
.gm__card-add:hover:not(:disabled),.gm__card-add.is-added { color:var(--silk); border-color:var(--crimson-h); }
.gm__card-add:disabled { opacity:.22; cursor:not-allowed; }
.gm__card-add svg { position:relative; z-index:1; }

/* Skeleton */
.gm__card--skel { pointer-events:none; animation:none !important; opacity:1 !important; transform:none !important; border-right:.5px solid var(--hair2); border-bottom:.5px solid var(--hair2); }
.gm__skel-img { aspect-ratio:4/3; background:linear-gradient(90deg,rgba(155,16,16,.04) 25%,rgba(155,16,16,.08) 50%,rgba(155,16,16,.04) 75%); background-size:200% 100%; animation:shimmerSkel 1.8s infinite; }
.gm__sk-line { height:8px; border-radius:1px; margin-bottom:7px; background:linear-gradient(90deg,rgba(155,16,16,.04) 25%,rgba(155,16,16,.08) 50%,rgba(155,16,16,.04) 75%); background-size:200% 100%; animation:shimmerSkel 1.8s infinite; }
...
/* ══ KILL FEED (list mode) — exclusivo gamer ══ */
.gm__killfeed { border-top:.5px solid var(--hair2); }
.gm__kf-header {
  display:grid; grid-template-columns:32px 44px 1fr 180px 64px 80px 120px;
  gap:0; padding:6px 14px; border-bottom:.5px solid rgba(155,16,16,.2);
  background:rgba(155,16,16,.04);
}
...
.gm__kf-row {
  display:grid; grid-template-columns:32px 44px 1fr 180px 64px 80px 120px;
  gap:0; border-bottom:.5px solid var(--hair2); align-items:center;
...
.gm__kf-row:hover { background:rgba(155,16,16,.03); }
.gm__kf-row--destaque { background:rgba(155,16,16,.04); }
.gm__kf-row--destaque::before { content:''; position:absolute; left:0; top:0; bottom:0; width:2px; background:var(--crimson-h); }
.gm__kf-row--esgotado { opacity:.5; }
/* scanline ao hover */
.gm__kf-row-scan { position:absolute; top:0; left:0; right:0; height:1px; background:linear-gradient(90deg,transparent,var(--crimson-h),transparent); opacity:0; pointer-events:none; transition:opacity .2s; }
.gm__kf-row:hover .gm__kf-row-scan { opacity:.35; }

.gm__kf-num { font-family:var(--f-mono); font-size:9px; color:var(--gold); opacity:.4; padding:14px 0 14px 14px; }
.gm__kf-img { width:44px; height:44px; flex-shrink:0; position:relative; overflow:hidden; background:var(--void); display:flex; align-items:center; justify-content:center; }
.gm__kf-img-fb { font-family:var(--f-title); font-size:1.2rem; color:var(--crimson); opacity:.2; position:absolute; }
...
.gm__kf-spec-tag { font-family:var(--f-mono); font-size:6px; letter-spacing:2px; text-transform:uppercase; background:rgba(200,168,75,.08); color:var(--gold); border:.5px solid rgba(200,168,75,.2); padding:2px 6px; display:inline-block; width:fit-content; }
.gm__kf-spec-tag--dest { background:rgba(155,16,16,.1); color:var(--crimson-h); border-color:rgba(155,16,16,.3); }
...
.gm__kf-tier-badge { font-family:var(--f-mono); font-size:10px; font-weight:700; width:28px; height:28px; display:flex; align-items:center; justify-content:center; }
.gm__kf-tier-badge--s { background:linear-gradient(135deg,#7a1111,var(--crimson-h)); color:rgba(237,232,224,.9); }
.gm__kf-tier-badge--a { background:rgba(155,16,16,.15); color:var(--crimson-h); border:.5px solid rgba(155,16,16,.4); }
.gm__kf-tier-badge--b { background:rgba(237,232,224,.03); color:var(--silk3); border:.5px solid var(--hair2); }
.gm__kf-tier-badge--c { background:transparent; color:var(--silk4); border:.5px solid var(--hair2); }
.gm__kf-xp-mini { width:44px; height:1.5px; background:rgba(237,232,224,.06); }
.gm__kf-xp-fill { height:100%; background:linear-gradient(90deg,var(--crimson),var(--crimson-h)); }
.gm__kf-preco-wrap { padding:14px 8px; }
.gm__kf-preco  { font-family:var(--f-title); font-size:13px; font-weight:700; color:var(--crimson-h); }
.gm__kf-parcela { font-family:var(--f-mono); font-size:7px; color:var(--gold-dim); margin-top:2px; }
.gm__kf-acao { padding:14px; display:flex; align-items:center; gap:6px; }
.gm__kf-status { display:flex; align-items:center; gap:4px; font-family:var(--f-mono); font-size:6px; letter-spacing:1.5px; color:var(--silk4); }
.gm__kf-status-dot { width:4px; height:4px; border-radius:50%; background:var(--silk4); flex-shrink:0; }
.gm__kf-status--on .gm__kf-status-dot { background:#22c55e; animation:dotBlink 1.3s ease-in-out infinite; }
.gm__kf-status--on { color:rgba(34,197,94,.7); }
.gm__kf-status--off .gm__kf-status-dot { background:rgba(239,68,68,.6); }
.gm__kf-status--off { color:rgba(239,68,68,.5); }
.gm__kf-add,.gm__kf-wish { width:28px; height:28px; background:transparent; border:.5px solid var(--hair2); color:var(--crimson-h); cursor:pointer; display:flex; align-items:center; justify-content:center; transition:all .25s; position:relative; overflow:hidden; }
.gm__kf-add:hover:not(:disabled),.gm__kf-add.is-added { background:var(--crimson); border-color:var(--crimson-h); color:var(--silk); }
.gm__kf-add:disabled { opacity:.2; cursor:not-allowed; }
.gm__kf-wish:hover,.gm__kf-wish.is-active { background:rgba(155,16,16,.2); border-color:var(--crimson); color:var(--crimson-h); }

/* ══ PAGINAÇÃO ══ */
.gm__pag-sep { display:flex; align-items:center; gap:16px; margin:44px 0 24px; }
.gm__pag-sep-line { flex:1; height:.5px; background:linear-gradient(90deg,transparent,rgba(155,16,16,.2)); }
.gm__pag-sep-line--r { background:linear-gradient(to left,transparent,rgba(155,16,16,.2)); }
.gm__pag-sep-kanji { font-family:var(--f-jp); font-size:11px; font-weight:200; color:var(--gold); opacity:.28; flex-shrink:0; line-height:1; }
.gm__paginacao { display:flex; justify-content:center; align-items:center; gap:5px; margin-bottom:12px; }
.gm__pag-btn { width:36px; height:36px; background:transparent; border:.5px solid var(--hair2); color:var(--silk4); cursor:pointer; font-family:var(--f-title); font-size:10px; display:flex; align-items:center; justify-content:center; transition:all .25s; position:relative; overflow:hidden; }
...
.gm__pag-btn:hover:not(:disabled) { color:var(--silk); border-color:var(--crimson-h); }
.gm__pag-btn.is-active { background:rgba(155,16,16,.12); border-color:var(--crimson-h); color:var(--crimson-h); }
...
/* ══ TOAST ══ */
.gm-toast { position:fixed; bottom:24px; right:24px; z-index:99999; display:flex; align-items:center; gap:10px; min-width:260px; max-width:320px; background:var(--deep); border:.5px solid var(--hair); padding:13px 14px; overflow:hidden; box-shadow:0 12px 36px rgba(0,0,0,.55); }
.gm-toast::before { content:''; position:absolute; top:0; left:0; right:0; height:.5px; background:linear-gradient(90deg,transparent,var(--toast-accent,var(--crimson-h)),transparent); opacity:.8; }
.gm-toast--success { --toast-accent:var(--gold); }
.gm-toast--info    { --toast-accent:var(--crimson-h); }
.gm-toast--error   { --toast-accent:#ef4444; }
.gm-toast__icon { width:22px; height:22px; border-radius:50%; display:flex; align-items:center; justify-content:center; background:color-mix(in srgb,var(--toast-accent,var(--crimson-h)) 14%,transparent); color:var(--toast-accent,var(--crimson-h)); flex-shrink:0; }
...
.gm-toast__progress { position:absolute; bottom:0; left:0; height:1px; background:var(--toast-accent,var(--crimson-h)); opacity:.5; width:100%; transform-origin:left; animation:toastProgress linear 1 forwards; }
...
/* ══ MODAL ══ */
.gm__modal-bg { position:fixed; inset:0; z-index:9000; background:rgba(0,0,0,.88); backdrop-filter:blur(22px); display:flex; align-items:center; justify-content:center; padding:18px; }
.modal-fade-enter-active,.modal-fade-leave-active { transition:opacity .3s; }
.modal-fade-enter-from,.modal-fade-leave-to { opacity:0; }
.gm__modal { background:var(--deep); border:.5px solid rgba(155,16,16,.25); position:relative; max-width:1000px; width:100%; max-height:92vh; overflow-y:auto; box-shadow:0 0 0 1px var(--hair2),0 60px 120px rgba(0,0,0,.7),0 0 60px rgba(155,16,16,.06); scrollbar-width:thin; scrollbar-color:rgba(155,16,16,.15) transparent; }
.gm__modal::-webkit-scrollbar { width:4px; }
.gm__modal::-webkit-scrollbar-thumb { background:rgba(155,16,16,.2); }
...
.gm__modal-realm-line { position:absolute; top:0; left:0; right:0; height:.5px; background:linear-gradient(90deg,transparent,var(--gold2) 20%,var(--crimson-h) 50%,var(--gold2) 80%,transparent); opacity:.7; z-index:2; }
/* Scanline no modal — exclusivo */
.gm__modal-scan { position:absolute; top:0; left:0; right:0; height:1.5px; background:linear-gradient(90deg,transparent,var(--crimson-h),transparent); opacity:.3; z-index:3; animation:modalScan 4s ease-in-out infinite; pointer-events:none; }
@keyframes modalScan { 0%,100%{top:0;opacity:.4} 50%{top:calc(100% - 1.5px);opacity:.1} }
.gm__mc { position:absolute; z-index:10; width:20px; height:20px; border-color:rgba(200,168,75,.2); border-style:solid; pointer-events:none; transition:border-color .4s; }
...
.gm__modal:hover .gm__mc { border-color:rgba(200,168,75,.45); }
.gm__modal-close-fixed { position:fixed; top:18px; right:18px; z-index:9100; width:36px; height:36px; border-radius:50%; background:rgba(5,5,12,.92); backdrop-filter:blur(8px); border:.5px solid rgba(155,16,16,.3); color:var(--silk); cursor:pointer; display:flex; align-items:center; justify-content:center; transition:all .25s; }
.gm__modal-close-fixed:hover { background:rgba(155,16,16,.15); color:var(--crimson-h); border-color:var(--crimson-h); transform:scale(1.07) rotate(90deg); }
.gm__modal-layout { display:grid; grid-template-columns:50% 50%; position:relative; z-index:1; min-height:520px; }

/* Col imagem */
.gm__modal-img-col { display:flex; flex-direction:column; background:rgba(0,0,0,.15); border-right:.5px solid rgba(155,16,16,.08); }
.gm__modal-viewer  { position:relative; flex:1; min-height:320px; overflow:hidden; background:var(--void); }
.gm__modal-fallback { position:absolute; inset:0; z-index:1; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:10px; background:linear-gradient(135deg,var(--void) 0%,rgba(155,16,16,.025) 100%); pointer-events:none; }
.gm__mf-letra { font-family:var(--f-title); font-size:4rem; font-weight:900; color:var(--crimson); opacity:.14; }
...
.gm__modal-img.is-zoom { transform:scale(1.8); cursor:zoom-out; z-index:5; }
.gm__modal-zoom-hint { position:absolute; bottom:10px; right:10px; z-index:6; font-family:var(--f-mono); font-size:7px; letter-spacing:2px; color:var(--silk4); background:rgba(0,0,0,.6); padding:3px 8px; display:flex; align-items:center; gap:4px; opacity:.6; pointer-events:none; }
.gm__modal-badges { position:absolute; top:12px; left:12px; z-index:6; display:flex; flex-direction:column; gap:4px; }
.gm__modal-badge { font-family:var(--f-title); font-size:6px; font-weight:700; letter-spacing:2.5px; text-transform:uppercase; padding:3px 8px; display:inline-flex; align-items:center; gap:4px; }
.gm__modal-badge--dest { background:var(--crimson); color:var(--silk); border-left:2px solid var(--gold); }
.gm__modal-badge--rgb  { background:rgba(200,168,75,.1); color:var(--gold); border:.5px solid var(--gold-dim); }
.gm__modal-badge--esgo { background:rgba(0,0,0,.75); border:.5px solid var(--hair2); color:var(--silk4); }

/* Tier strip no modal viewer — exclusivo */
.gm__modal-tier-strip { position:absolute; bottom:0; left:0; right:0; z-index:6; background:rgba(5,5,12,.85); border-top:.5px solid var(--hair2); padding:8px 12px; display:flex; align-items:center; gap:12px; }
.gm__modal-tier { font-family:var(--f-mono); font-size:6.5px; letter-spacing:2px; font-weight:700; padding:2px 8px; flex-shrink:0; }
.gm__modal-tier--s { background:linear-gradient(135deg,#7a1111,var(--crimson-h)); color:rgba(237,232,224,.9); border-left:2px solid var(--gold); }
.gm__modal-tier--a { background:rgba(155,16,16,.15); color:var(--crimson-h); border:.5px solid rgba(155,16,16,.4); }
.gm__modal-tier--b { background:rgba(237,232,224,.03); color:var(--silk3); border:.5px solid var(--hair2); }
...
.gm__modal-xp-track { height:2px; background:rgba(237,232,224,.06); }
.gm__modal-xp-fill { height:100%; background:linear-gradient(90deg,var(--crimson),var(--crimson-h)); position:relative; }
.gm__modal-xp-fill::after { content:''; position:absolute; right:-1px; top:-2px; width:4px; height:6px; background:var(--gold); opacity:.85; }

.gm__modal-thumbs { display:flex; gap:4px; padding:8px 10px 10px; border-top:.5px solid rgba(155,16,16,.06); background:rgba(0,0,0,.1); flex-wrap:wrap; }
.gm__modal-thumb { width:54px; flex-shrink:0; border:.5px solid var(--hair2); background:var(--void); cursor:pointer; display:flex; flex-direction:column; align-items:center; overflow:hidden; transition:all .25s; padding:0; }
.gm__modal-thumb:hover { border-color:var(--hair); }
.gm__modal-thumb.is-active { border-color:var(--crimson-h); box-shadow:0 0 0 1px rgba(155,16,16,.2); }
.gm__modal-thumb img { width:100%; height:38px; object-fit:cover; display:block; }
.gm__modal-thumb-fb { width:100%; height:38px; display:flex; align-items:center; justify-content:center; font-family:var(--f-title); font-size:.9rem; color:var(--crimson); opacity:.3; }
...
.gm__mo-line { flex:1; height:.5px; background:linear-gradient(90deg,var(--crimson-h),transparent); opacity:.2; }
.gm__mo-line--r { background:linear-gradient(270deg,var(--crimson-h),transparent); }
.gm__mo-kanji { font-family:var(--f-jp); font-size:14px; color:var(--gold); opacity:.25; line-height:1; flex-shrink:0; }
.gm__modal-eyebrow { display:flex; align-items:center; gap:8px; margin-bottom:10px; }
.gm__mey-dot { width:4px; height:4px; border-radius:50%; background:var(--crimson-h); opacity:.8; flex-shrink:0; }
.gm__mey-txt { font-size:7px; letter-spacing:6px; font-weight:700; text-transform:uppercase; background:linear-gradient(135deg,var(--gold),var(--gold2)); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
.gm__modal-titulo { font-family:var(--f-title); font-size:clamp(1.3rem,2vw,1.8rem); font-weight:700; color:var(--silk); line-height:1.1; margin-bottom:5px; letter-spacing:.03em; }
...
/* Ping row no modal — exclusivo gamer */
.gm__modal-ping-row { display:flex; align-items:center; gap:14px; padding:8px 0; margin:4px 0; border-top:.5px solid var(--hair2); border-bottom:.5px solid var(--hair2); }
...
.gm__modal-ping-dot--green { background:#22c55e; animation:dotBlink 1.3s ease-in-out infinite; }
...
.gm__md-line { flex:1; height:.5px; background:linear-gradient(90deg,transparent,rgba(155,16,16,.3)); }
.gm__md-line--r { background:linear-gradient(to left,transparent,rgba(155,16,16,.3)); }
.gm__md-gem { font-size:6px; color:rgba(200,168,75,.4); }
...
.gm__specs-hd::after { content:''; flex:1; height:.5px; background:linear-gradient(to right,rgba(155,16,16,.2),transparent); }
.gm__specs-kanji { font-family:var(--f-jp); font-size:13px; font-weight:200; color:var(--crimson-h); opacity:.65; letter-spacing:0; line-height:1; }
.gm__specs-list { display:flex; flex-direction:column; }
.gm__spec-row { display:flex; align-items:baseline; gap:7px; padding:7px 0; border-bottom:.5px solid rgba(155,16,16,.06); transition:background .2s,padding-left .2s; }
.gm__spec-row:first-child { border-top:.5px solid rgba(155,16,16,.06); }
.gm__spec-row:hover { background:rgba(155,16,16,.02); padding-left:5px; }
.gm__spec-k    { font-family:var(--f-mono); font-size:7px; letter-spacing:2px; text-transform:uppercase; color:var(--gold-dim); white-space:nowrap; flex-shrink:0; width:85px; }
.gm__spec-dots { flex:1; height:.5px; min-width:10px; background:repeating-linear-gradient(90deg,rgba(155,16,16,.15) 0,rgba(155,16,16,.15) 2px,transparent 2px,transparent 5px); margin-bottom:1px; }
...
.gm__modal-qty { display:flex; align-items:center; border:.5px solid var(--hair2); width:fit-content; }
...
.gm__qty-btn:hover:not(:disabled) { background:var(--crimson-h); color:var(--silk); }
.gm__qty-btn:disabled { opacity:.2; cursor:not-allowed; }
.gm__qty-val { font-family:var(--f-mono); font-size:11px; color:var(--silk); min-width:36px; height:28px; display:flex; align-items:center; justify-content:center; border-left:.5px solid var(--hair2); border-right:.5px solid var(--hair2); letter-spacing:.1em; }

/* Preço */
.gm__modal-preco-wrap { margin-bottom:20px; padding:16px 0; border-top:.5px solid rgba(155,16,16,.08); }
.gm__modal-preco-hd { display:flex; align-items:center; gap:7px; margin-bottom:7px; }
.gm__mp-kanji { font-family:var(--f-jp); font-size:16px; font-weight:200; color:var(--crimson-h); opacity:.5; line-height:1; }
...
.gm__mp-line  { flex:1; height:.5px; background:linear-gradient(to right,rgba(155,16,16,.2),transparent); }
.gm__modal-preco { font-family:var(--f-title); font-size:clamp(1.7rem,3vw,2.4rem); font-weight:700; color:var(--silk); line-height:1; display:flex; align-items:baseline; gap:7px; }
.gm__mp-rs    { font-family:var(--f-mono); font-size:.65em; color:var(--silk4); letter-spacing:2px; }
.gm__modal-preco em { font-style:normal; background:linear-gradient(135deg,var(--crimson-h) 0%,var(--ember) 50%,var(--gold) 100%); background-size:200% auto; -webkit-background-clip:text; background-clip:text; -webkit-text-fill-color:transparent; animation:mainShine 5s linear infinite; }
...
.gm__modal-parcela span { color:rgba(155,16,16,.4); }

/* Ações */
.gm__modal-actions { display:flex; gap:8px; margin-bottom:16px; flex-wrap:wrap; }
.gm__modal-add { flex:1; min-width:160px; position:relative; overflow:hidden; background:transparent; border:.5px solid var(--crimson-h); font-family:var(--f-title); font-size:7.5px; font-weight:700; letter-spacing:3.5px; color:var(--crimson-h); padding:13px 18px; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:8px; transition:color .45s; z-index:0; text-transform:uppercase; }
...
.gm__modal-add.is-added { border-color:var(--hair); color:var(--silk3); }
.gm__modal-add.is-added .gm__modal-add-fill { transform:none; background:transparent; }
.gm__modal-icon-btn { width:42px; flex-shrink:0; background:rgba(155,16,16,.05); border:.5px solid var(--hair2); display:flex; align-items:center; justify-content:center; cursor:pointer; transition:all .25s; color:var(--crimson-h); }
.gm__modal-icon-btn:hover,.gm__modal-icon-btn.is-active { border-color:var(--crimson); background:rgba(155,16,16,.12); transform:scale(1.05); }

/* Relacionados */
.gm__modal-rel { margin-top:20px; padding-top:16px; border-top:.5px solid rgba(155,16,16,.06); }
...
.gm__rel-kanji { font-family:var(--f-jp); font-size:11px; font-weight:200; color:var(--crimson-h); opacity:.65; letter-spacing:0; line-height:1; }
.gm__rel-line  { flex:1; height:.5px; background:linear-gradient(to right,rgba(155,16,16,.2),transparent); }
.gm__modal-rel-grid { display:grid; grid-template-columns:repeat(3,1fr); border-left:.5px solid var(--hair2); border-top:.5px solid var(--hair2); }
.gm__modal-rel-card { border-right:.5px solid var(--hair2); border-bottom:.5px solid var(--hair2); cursor:pointer; transition:background .25s; overflow:hidden; }
.gm__modal-rel-card:hover { background:rgba(155,16,16,.04); }
.gm__rel-img { height:68px; background:var(--void); position:relative; overflow:hidden; }
.gm__rel-fb  { position:absolute; inset:0; display:flex; align-items:center; justify-content:center; font-family:var(--f-title); font-size:1.8rem; font-weight:900; color:var(--crimson); opacity:.15; }
...
.gm__rel-preco { font-family:var(--f-title); font-size:11px; color:var(--crimson-h); font-weight:700; }

.gm__modal-footer-note { display:flex; align-items:center; justify-content:center; gap:8px; padding-top:14px; border-top:.5px solid var(--hair2); font-family:var(--f-mono); font-size:6px; letter-spacing:3px; color:var(--silk4); text-transform:uppercase; margin-top:auto; }
.gm__modal-footer-note svg { color:var(--gold); opacity:.35; flex-shrink:0; }
.gm__fn-kanji { font-family:var(--f-jp); font-size:12px; font-weight:200; color:var(--gold); opacity:.32; letter-spacing:0; line-height:1; }
...
@media (max-width:1100px) {
  .gm__hero { padding:calc(var(--navbar-h,88px)+44px) 44px 0; }
  .gm__hero-inner { grid-template-columns:1fr 250px; gap:40px; padding-left:calc(6% + 24px); }
  .gm__strip { margin:0 -44px; padding:0 44px; }
  .gm__kf-header,.gm__kf-row { grid-template-columns:28px 44px 1fr 120px 52px 72px 100px; }
}
@media (max-width:900px) {
  .gm__hero-inner { grid-template-columns:1fr; }
  .gm__hero-right { display:none; }
  .gm__modal-layout { grid-template-columns:1fr; }
  .gm__modal-img-col { border-right:none; border-bottom:.5px solid rgba(155,16,16,.06); }
  ...
  .gm__kf-header,.gm__kf-row { grid-template-columns:28px 44px 1fr 52px 72px 100px; }
  .gm__kf-specs { display:none; }
}
@media (max-width:768px) {
  .gm__sidebar { position:fixed !important; top:0 !important; left:0 !important; bottom:0 !important; width:290px; z-index:99; transform:translateX(-100%); padding:0; height:100vh !important; overflow-y:auto !important; }
  ...
  .gm__kf-row { grid-template-columns:28px 44px 1fr 64px 100px; }
  .gm__kf-tier,.gm__kf-preco-wrap { display:none; }
}
@media (max-width:480px) {
  .gm__hero { padding:calc(var(--navbar-h,88px)+24px) 16px 0; }
...
@media (prefers-reduced-motion:reduce) {
  ...
  .gm__card-scan,.gm__modal-scan,.gm__lat-b { animation:none !important; }
